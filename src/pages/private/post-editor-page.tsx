...
const PostEditorForm = ({
  ...
  const createMutation = useMutation({
    ...
    onError: () => {
      toast.error("Errore nella crezione dell'articolo")
    },
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, body }: { id: string, body: PostWriteBody }) => PostsService.update(id, body),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["posts"] });
  ...
  const updateFeaturedImageMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['posts-featured-image'] })
  ...
  const onSubmit = (_values: PostEditorFormValues) => {
    if (isEdit) {
      ...
      createMutation.mutate({ ..._values, userId: user!.id })
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button variant="outline" render={<Link to="/my-posts" />} nativeButton={false}>
          Cancel
        </Button>
      </div>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6" noValidate>
        ...
        <Field>
          ...
          <FieldDescription>
            Article text. Blog cards show a short preview; the article page shows
            the full text.
          </FieldDescription>
          <Textarea
            id="description"
        ...
        {isEdit && (
          <Field>
            ...
            {((!featuredImageError && featuredImageUrl) || form.watch('featuredImage')) && <img className="h-40 w-fit!" src={form.getValues('featuredImage') ? URL.createObjectURL(form.getValues('featuredImage')!) : featuredImageUrl} alt={initialPost?.title} />}
            <Controller name="featuredImage" control={form.control} render={({ field: { onChange, onBlur, name } }) => (
              <Input
                id="featured-image"
                name={name}
                onBlur={onBlur}
...
const PostEditorPage = () => {
  ...
  const { data: cateogoriesData } = useQuery({
    queryKey: ["categories"],
  ...
  return (
    <PostEditorForm
      ...
      categories={cateogoriesData?.data || []}
    />
  )
}
