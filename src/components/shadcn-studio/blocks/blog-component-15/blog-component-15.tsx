...
const BlogCard = ({ post }: { post: Post<{ user: User, categories: Category[] }> }) => {
          ...
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            <ScrollArea className="bg-muted w-full rounded-lg sm:w-auto">
              <TabsList className="gap-1 group-data-[orientation=horizontal]/tabs:h-auto">
                <TabsTrigger
                  value="all"
                  className="hover:bg-primary/10 cursor-pointer rounded-lg px-4 text-base group-data-[orientation=horizontal]/tabs:after:h-0"
                >
                  All
                </TabsTrigger>
                {categories.map((item) => (
        ...
        {postsPending ? (
          ...
          <Empty>
            <EmptyHeader>
              ...
              <EmptyDescription>La ricerca di "{searchDraft}" non ha prodotto nessun risultato!</EmptyDescription>
            </EmptyHeader>
          </Empty>
        )}
      </div>
