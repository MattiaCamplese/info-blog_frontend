...
const HeroSection = ({ blogData }: { blogData: Post<{ user: User, categories: Category[] }>[] }) => {
  return (
    <section className="bg-muted pt-24 pb-12 sm:pb-16 lg:pb-24">
      <div className="mx-auto flex h-full max-w-7xl flex-col gap-16 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {blogData.map((post) => <HeroCard key={post.id} post={post} />)}
        </div>
      </div>
    </section>
  )
...
const HeroCard = ({ post }: {post : Post<{ user: User; categories: Category[] }> }) => {
      ...
      <CardContent className="grid grid-cols-1 px-0 xl:grid-cols-2">
        ...
        <div className="flex flex-col justify-center gap-3 p-6">
          <div className="flex items-center gap-1.5 py-1">
            ...
            {post.categories.map(category => <Badge key={category.id} className="bg-primary/10 text-primary border-0 text-sm">{category.name}</Badge>)}
          </div>
